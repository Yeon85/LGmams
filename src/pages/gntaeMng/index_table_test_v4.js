...
const Calender = props => {
  ...
  const toggleCategory_2 = (id) => {
    ...
    alert(click);
    console.log(click);
      return { ...row, name: "2025-06-16",  position: "연수" };
   // }
  ...
  setData_2({ ...data_2, rows: updatedRows }); // 변경된 데이터로 상태 업데이트
    ...
    //setModalcategory_2(!modalcategory_2);

    // 클릭된 버튼에 해당하는 데이터 값을 변경합니다.

    //data_2.rows.ma
    /* const updatedData = data_2.rows.map(item => {
     // if (item.id === id) {
        return { ...item, name: "2025-06-16",  position: "연수", };
     // }
      return item;
    });

    setData_2(updatedData); // 변경된 데이터로 상태 업데이트
