...
const Calender = props => {
  ...
  //추가_정연주 20240303
  const [modalcategory_2, setModalcategory_2] = useState(false);
  ...
  const toggleCategory_2 = (id) => {

    // 클릭된 버튼에 해당하는 데이터 값을 변경합니다.
    const updatedRows = data_2.rows.map(row => {
      
      setCount(count + 1); 
      if (count === 1) {
        return { ...row, name: "2024-01-01",  position: "연수" };
      }
      if (count === 2) {
          return { ...row, name: "2024-02-02",  position: "휴가(일반)" };
      }
      if (count === 3) {
        return { ...row, name: "2024-03-03",  position: "리프레쉬휴가" };
      }
      if (count === 4) {
        return { ...row, name: "2024-04-08",  position: "경조사" };
      } 

      if (count === 5) {
        return { ...row, name: "2024-05-09",  position: "병원방문" };
      }

    if (count === 6) {
      return { ...row, name: "2024-06-18",  position: "본사방문" };
    }
   
    if (count === 7) {
      return { ...row, name: "2024-07-19",  position: "병원방문" };
    }
    if (count === 8) {
      return { ...row, name: "2024-08-20",  position: "미용실방문" };

    }
    if (count === 9) {
      return { ...row, name: "2024-09-19",  position: "가사" };
    }

    if (count === 10) {
      return { ...row, name: "2024-10-22",  position: "개인휴가" };
    }


   // if (row.id === id) {
   //   return { ...row, name: "2024-01-08",  position: "가사" };
   // }
    return row;
  });

  setData_2({ ...data_2, rows: updatedRows }); // 변경된 데이터로 상태 업데이트
  };
    ...
    rows: [
      {
        id: 1,
        value: '데이터 1',
        button: <button onClick={() => handleClick(1)}>변경</button>
      },
      {
        id: 2,
        value: '데이터 2',
        button: <button onClick={() => handleClick(2)}>변경</button>
      },
      {
        id: 3,
        value: '데이터 3',
        button: <button onClick={() => handleClick(3)}>변경</button>
      }
    ]
  });

const handleClick = (id) => {
  /*  // 클릭된 행에 해당하는 데이터 값을 가져옵니다.
   const clickedRow = data.rows.find(row => row.id === id);
   if (clickedRow) {
     console.log(clickedRow.value); // 클릭된 행에 해당하는 값을 출력하거나 원하는 동작을 수행합니다.
   }   */

 // 클릭된 버튼에 해당하는 데이터 값을 변경합니다.
  const updatedRows = data2.rows.map(row => {
    if (row.id === id) {
      return { ...row, value: '변경된 값' };
    }
    return row;
  });

setData2({ ...data2, rows: updatedRows }); // 변경된 데이터로 상태 업데이트
};
    ...
    <React.Fragment>
      ...
      <div className="page-content">
        <Container fluid={true}>
          {/* Render Breadcrumb */}
          <Breadcrumbs maintitle="자율운영"  title="작업시간준수" breadcrumbItem="연차현황2" />
          <Row>
