...
const onDrop = useCallback((acceptedFiles) => {
  console.log("onDropfrom:"+disableClick);
  setFiles([...files, ...acceptedFiles]);
  console.log("onDropto:"+disableClick);
}, [files]);
...
setDisableClick(updatedFiles.some((file)=> !file.deleteFile))
  ...
  const handleDropzoneChange = () => {
    // disableClick 값을 변경하고 싶을 때
    setDisableClick(true);
    console.log("hhhhdisableClick:"+disableClick);
  };
...
const onDragEnterHandler = () => {
  console.log('드롭된 파일:');
  // Dropzone에 파일이 들어갈 때 배경색을 변경하는 동작 수행
  // 예시: 배경색을 노란색으로 변경
  document.getElementById('dropzone').style.backgroundColor = 'yellow';
};

const onDragLeaveHandler = () => {
  // Dropzone을 벗어날 때 배경색을 원래대로 복원하는 동작 수행
  // 예시: 배경색을 흰색으로 변경
  console.log('드롭된 파일:');
...
const onDropHandler = (acceptedFiles) => {
  // 파일이 Dropzone에 드롭될 때 수행할 동작 수행
  console.log('드롭된 파일:', acceptedFiles);
};
//파일업로드 버튼1
